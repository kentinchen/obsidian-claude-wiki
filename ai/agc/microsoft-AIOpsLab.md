---
title: "microsoft/AIOpsLab: A holistic framework to enable the design, development, and evaluation of autonomous AIOps agents."
source: "https://github.com/microsoft/AIOpsLab/"
author:
published:
created: 2026-04-17
description: "A holistic framework to enable the design, development, and evaluation of autonomous AIOps agents. - microsoft/AIOpsLab"
tags:
  - "clippings"
---
## AIOpsLab

[🤖Overview](#%F0%9F%A4%96overview) | [🚀Quick Start](#%F0%9F%9A%80quickstart) | [📦Installation](#%F0%9F%93%A6installation) | [⚙️Usage](#%E2%9A%99%EF%B8%8Fusage) | [📂Project Structure](#%F0%9F%93%82project-structure) | [📄How to Cite](#%F0%9F%93%84how-to-cite)

## 🤖 Overview

[![alt text](https://github.com/microsoft/AIOpsLab/raw/main/assets/images/aiopslab-arch-open-source.png)](https://github.com/microsoft/AIOpsLab/blob/main/assets/images/aiopslab-arch-open-source.png)
...
Moreover, AIOpsLab provides a built-in benchmark suite with a set of problems to evaluate AIOps agents in an interactive environment. This suite can be easily extended to meet user-specific needs. See the problem list [here](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/problems/registry.py#L15).
...
AIOpsLab can be run on a local simulated cluster using [kind](https://kind.sigs.k8s.io/) on your local machine. Please look at this [README](https://github.com/microsoft/AIOpsLab/blob/main/kind/README.md#prerequisites) for a list of prerequisites.
...
If you're running into issues, consider building a Docker image for your machine by following this [README](https://github.com/microsoft/AIOpsLab/blob/main/kind/README.md#deployment-steps). Please also open an issue.
...
See [Terraform README](https://github.com/microsoft/AIOpsLab/blob/main/scripts/terraform/README.md) for all options (`--allowed-ips`, `--dev`, `--setup-only`, etc.).
...
Our repository comes with a variety of pre-integrated agents, including agents that enable **secure authentication with Azure OpenAI endpoints using identity-based access**. Please check out [Clients](https://github.com/microsoft/AIOpsLab/blob/main/clients) for a comprehensive list of all implemented clients.
...
2. Set the USE\_WANDB environment variable:
	```
...
3. **Evaluate your agent on a problem**:
	1. **Initialize a problem**: AIOpsLab provides a list of problems that you can evaluate your agent on. Find the list of available problems [here](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/problems/registry.py) or using `orch.probs.get_problem_ids()`. Now initialize a problem by its ID:
		```
...
This process will create a [`Session`](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/session.py) with the orchestrator, where the agent will solve the problem. The orchestrator will evaluate your agent's solution and provide results (stored under `data/results/`). You can use these to improve your agent.

### How to add new applications to AIOpsLab?

AIOpsLab provides a default [list of applications](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/service/apps) to evaluate agents for operations tasks. However, as a developer you can add new applications to AIOpsLab and design problems around them.

> *Note*: for auto-deployment of some apps with K8S, we integrate Helm charts (you can also use `kubectl` to install as [HotelRes application](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/service/apps/hotelres.py)). More on Helm [here](https://helm.sh/).
...
1. **Add application metadata**
	...
	Create a JSON file with this metadata and save it in the [`metadata`](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/service/metadata) directory. For example the `social-network` app: [social-network.json](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/service/metadata/social-network.json)
2. **Add application class**
	Extend the base class in a new Python file in the [`apps`](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/service/apps) directory:
	```
...
Similar to applications, AIOpsLab provides a default [list of problems](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/problems/registry.py) to evaluate agents. However, as a developer you can add new problems to AIOpsLab and design them around your applications.

Each problem in AIOpsLab has 5 components:

1. *Application*: The application on which the problem is based.
2. *Task*: The AIOps task that the agent needs to perform. Currently we support: [Detection](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/tasks/detection.py), [Localization](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/tasks/localization.py), [Analysis](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/tasks/analysis.py), and [Mitigation](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/tasks/mitigation.py).
...
To add a new problem to AIOpsLab, create a new Python file in the [`problems`](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/problems) directory, as follows:
...
2. **Define**. To define a problem, create a class that inherits from your chosen `Task`, and defines 3 methods: `start_workload`, `inject_fault`, and `eval`:
	...
	```
3. **Register**. Finally, add your problem to the orchestrator's registry [here](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/problems/registry.py).

See a full example of a problem [here](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/problems/k8s_target_port_misconfig/target_port.py).
...
- **`start_workload`**: Initiates the application's workload. Use your own generator or AIOpsLab's default, which is based on [wrk2](https://github.com/giltene/wrk2):
	...
	```
	> Relevant Code: [aiopslab/generators/workload/wrk.py](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/generators/workload/wrk.py)
- **`inject_fault`**: Introduces a fault into the application. Use your own injector or AIOpsLab's built-in one which you can also extend. E.g., a misconfig in the K8S layer:
	...
	```
	> Relevant Code: [aiopslab/generators/fault](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/generators/fault)
- **`eval`**: Evaluates the agent's solution using 3 params: (1) *soln*: agent's submitted solution if any, (2) *trace*: agent's action trace, and (3) *duration*: time taken by the agent.
	...
	def eval(self, soln, trace, duration) -> dict:
	    ...
	    return self.results
	```
	> *Note*: When an agent starts a problem, the orchestrator creates a [`Session`](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/session.py) object that stores the agent's interaction. The `trace` parameter is this session's recorded trace.
	> Relevant Code: [aiopslab/orchestrator/evaluators/](https://github.com/microsoft/AIOpsLab/blob/main/aiopslab/orchestrator/evaluators)
...
={Yinfang Chen and Manish Shetty and Gagan Somashekar and Minghua Ma and Yogesh Simmhan and Jonathan Mace and Chetan Bansal and Rujia Wang and Saravan Rajmohan},
...
@inproceedings{shetty2024building,
  title = {Building AI Agents for Autonomous Clouds: Challenges and Design Principles},
   = {Shetty, Manish and Chen, Yinfang and Somashekar, Gagan and Ma, Minghua and Simmhan, Yogesh and Zhang, Xuchao and Mace, Jonathan and Vandevoorde, Dax and Las-Casas, Pedro and Gupta, Shachee Mishra and Nath, Suman and Bansal, Chetan and Rajmohan, Saravan},
  year = {2024},
...
Licensed under the [MIT](https://github.com/microsoft/AIOpsLab/blob/main/LICENSE.txt) license.
