...
2、软件安装： git、docker、nvm、node
    yum install -y git docker docker-compose-plugin FlyEnv-4.17.1-x64.rpm 
...
3、安装ansible
	...
	ssh-copy-id root@172.63.132.141
	pip install ansible -i https://pypi.tuna.tsinghua.edu.cn/simple/ --trusted-host pypi.tuna.tsinghua.edu.cn
	docker save registry.istio.io/release/proxyv2:1.30.3 > image.tar
	ansible k8s-w -m copy  -a 'src=image.tar dest=/tmp/'
	ansible k8s-w -m shell -a 'ctr -n k8s.io image import /tmp/image.tar'
	ansible k8s-w -m shell -a 'crictl images ls|grep proxyv2'
...
4、安装buzz
    yum install glib2-devel gtk3-devel libsoup3-devel pkgconf-pkg-config javascriptcoregtk4.1.x86_64
